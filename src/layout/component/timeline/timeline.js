...
let zzz=0;
  $('.timeline__year').click(function () {
        ...
        $('.timeline__year').removeClass('_active')
        $(this).addClass('_active')
        $('.timeline__bg').css('left', $(this).position().left)
	  var year = $(this).data('year')
	  $('body').attr('id','year-now-'+year)
    $('.map__layer').stop().fadeOut(100).removeClass('_open')
        let $thLayer = $('#' + $('.type-map__item._selected').attr('id') + '-' + $('.timeline__year._active').data('year'));
        let $thLayerClass = $('.' + $('.type-map__item._selected').attr('id') + '-' + $('.timeline__year._active').data('year'));
      $thLayer.stop().delay(500).fadeIn(1000, function() {$thLayer.addClass('_open')})
      $thLayerClass.stop().delay(500).fadeIn(1000, function() {$thLayerClass.addClass('_open')})

		$('#map-river path').each(function () {
			let shapeIndex = '';
			...
			if ($(this).attr('d') != $(thisId + '-' + year).attr('d')) {
				gsap.to($(thisId)[0], { duration: 1, morphSVG:{ shape: thisId + '-' + year , shapeIndex: shapeIndex}})
			}
        ...
        $('#map path').each(function () {
            ...
            if ($(this).attr('d') != $(thisId + '-' + year).attr('d')) {
                // findShapeIndex(thisId, thisId + '-' + year);
                gsap.to($(thisId)[0], {duration: 1, morphSVG: thisId + '-' + year})
                // if('#country-1-1950'==thisId + '-' + year){
                // KUTE.to(thisId, { path: thisId + '-' + year }, {
                //     easing: 'linear',
                //     duration: 300,
                //     morphPrecision:8
                // }).start();
                ...
                //     }).start();
                // }
                // $(this).animate({ d: $(thisId+'-'+year).attr('d') }, 1000, mina.bounce);
            }
        })
        
    })

    $('.timeline__year').first().click();
}
